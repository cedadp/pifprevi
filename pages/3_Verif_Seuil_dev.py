...
def to_excel_bytes(df, sheet_name="Data"):
    buffer = io.BytesIO() 
    with pd.ExcelWriter(buffer, engine="openpyxl") as writer: df.to_excel(writer, index=False, sheet_name=sheet_name) 
...
# calcul du cumul de charge horaire glissant à partir de données 10 min
    ...
    # retourne le dataframe dans un format "long"
        ...
        st.download_button( label="⬇️ Télécharger les données cumul glissant horaire", data=to_excel_bytes(df_pivot, sheet_name="Data"), file_name="data.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", )
